...
char publicFiles[PUBLIC_PIPE_SIZE][7];
...
void initPublicFiles()
{
	...
	{
		sprintf(publicFiles[i], "../%d", i);
		FILE *f = fopen(publicFiles[i], "r");
		if(f)
		{
			fclose(f);
			if(remove(publicFiles[i]))
...
int command_handler(int sockfd, char *cmd, int id, int pipes[PIPE_SIZE][2], int &pipePtr)
{
	...
	for(int i = 0; i < cmdCount - isPipe - isErrorPipe - isFilePipe - isAllPipe; ++i)
	{
		...
		if(isPublicPipeIn)
		{
			FILE *f = fopen(publicFiles[publicPipeIn], "r");
			if(!f)
			{
		...
		if(isPublicPipeOut)
		{
			FILE *f = fopen(publicFiles[publicPipeOut], "r");
			if(f)
			{
				fclose(f);
		...
		if(isPublicPipeIn)
		{
			sprintf(msg, publicPipeInStr, cname + (id * NAME_LENGTH), id + 1, cmd);
			cmdYell(msg, strlen(msg), id, true);
		}
		if(isPublicPipeOut)
		{
			sprintf(msg, publicPipeOutStr, cname + (id * NAME_LENGTH), id + 1, cmd);
			cmdYell(msg, strlen(msg), id, true);
		}
		...
		else if(execpid == 0) //exec child
		{
			...
			if(isPublicPipeOut)
			{
				FILE *f = fopen(publicFiles[publicPipeOut], "w");
				dup(fileno(f));
			}
			else if(isLastCmd)
			{
		...
		else
		{
			...
			if(isPublicPipeIn)
				remove(publicFiles[publicPipeIn]);
...
int main(int argc, char *argv[], char ** envp){
	...
	while(1)
	{
		...
		{
			cpid[newId] = childpid;
			printf ("server: fork finished, cpid = %d(%s/%s),ssock=%d\n", childpid, cip + (newId * IP_LENGTH), cport + (newId * PORT_LENGTH), newsockfd);
		}
	}
